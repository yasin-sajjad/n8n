/**
 * Plan Quality Experiment - CLI Entry Point
 *
 * Usage:
 *   pnpm eval:plans --dataset "plan-quality" --name "PQ-v1"
 *   pnpm eval:plans --local
 *   pnpm eval:plans --seed --dataset "plan-quality"
 */
...
main().catch((error) => {
	...
	console.error(`Plan quality evaluation failed: ${message}`);
	process.exit(1);
});
