...
const MAIN_WAIT_STRATEGY = Wait.forAll([
	...
	Wait.forHttp('/healthz/readiness', 5678).forStatusCode(200).withStartupTimeout(120000),
	Wait.forLogMessage('Editor is now accessible via').withStartupTimeout(120000),
]);

const WORKER_WAIT_STRATEGY = Wait.forAll([
	Wait.forListeningPorts(),
	Wait.forLogMessage('n8n worker is now ready').withStartupTimeout(120000),
]);
